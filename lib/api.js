	...
	this.source = String(src)
		.replace(/{cdata}([\s\S]*?){(?:\/cdata|end cdata)}/gm, function(sstr, data)  {
			...
			return '' +
				...
				(("__SNAKESKIN_CDATA__" + (this$0.cDataContent.length - 1)) + "_")
			;
		});
...
/**
 * Вернуть строку начала конкатенации c __SNAKESKIN_RESULT__
 * @return {string}
 */
...
/**
 * Вернуть строку окончания конкатенации c __SNAKESKIN_RESULT__
 * @return {string}
 */
DirObj.prototype.$$ = function () {
...
/**
 * Вернуть строку конкатенации c __SNAKESKIN_RESULT__
 *
