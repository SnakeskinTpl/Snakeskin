...
/**
 * Очистить "пустые" конструкции в результирующей строке JavaScript
 * @retur {!DirObj}
 */
DirObj.prototype.clearEmptyDirs = function () {
	switch (this.renderMode) {
		case 'stringBuffer':
	...
	this.res = this.pasteDangerBlocks(this.res)
		.replace(
			...
			(sstr, pos) => escapeNextLine(
					this.cDataContent[pos].replace(new RegExp(nextLineRgxp.source, 'g'), nl)
				).replace(/'/g, '&#39;')
		);
