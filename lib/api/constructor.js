...
export class Parser {
	...
	construtor(src, params) {
		...
		/**
		 * The map of directives, which can go after the initial
		 * @type {Object}
		 */
		this.after = null;

		/**
		 * If is false, then themplate won't inserted into the resulting JS string
		 * @type {boolean}
		 */
		this.canWrite = true;
		...
		/**
		 * If is true, then a directive is inline
		 * @type {?boolean}
		 */
		this.inline = null;

		/**
		 * If is true, then a directive has a text type
		 * @type {boolean}
		 */
		this.text = false;
