...
/**
 * Adds a new directive to the Snakeskin namespace
 *
...
Snakeskin.addDirective = function (name, params, constr, opt_destr) {
	...
	$C([
		_([afterDir, params.after]),
		_([insideDir, params.inside]),
		_([blockDirs, params.block, Boolean]),
		_([sysDirs, params.sys, Boolean]),
		_([textDirs, params.text, Boolean])
	...
	$C(Array.isArray(params.group) ? params.group : [params.group]).forEach((group) => {
		...
		dirGroups[group][name] = true;
		dirGroupsList[group].push(`"${name}"`);
	});

	$C(Array.isArray(params.chain) ? params.chain : [params.chain]).forEach((chain) => {
		dirChain[chain] = dirChain[chain] || {};
		dirChain[chain][name] = true;
	});

	$C(Array.isArray(params.end) ? params.end : [params.end]).forEach((end) => {
		dirEnd[end] = dirEnd[end] || {};
		dirEnd[end][name] = true;
	});
	...
	Snakeskin.Directives[name] = function (command, commandLength, type, jsDoc) {
		...
		if (struct.strong) {
			if (insideDir[struct.name][dirName]) {
		...
		const newStruct = dir.structure;

		if (insideDir[dirName]) {
		...
		if (dirName === sourceName) {
			if (struct === newStruct) {
				if (!ignore && afterDir[struct.name] && !afterDir[struct.name][dirName]) {
				...
				if (siblings) {
					...
					if (!ignore && prev && afterDir[prev.name] && !afterDir[prev.name][dirName]) {
	...
	const baseEnd = Snakeskin.Directions[`${name}BaseEnd`] = function () {
