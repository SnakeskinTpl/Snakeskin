...
import { forEach } from './deps/collection';
import {
...
Snakeskin.addDirective = function (name, params, constr, opt_destr) {
	...
	params.replacers::forEach((el, key) => {
		if (key.length > 2) {
	...
	(Array.isArray(params.group) ? params.group : [params.group])::forEach((group) => {
		if (!DIR_GROUPS[group]) {
			DIR_GROUPS[group] = {};
	...
	(Array.isArray(params.chain) ? params.chain : [params.chain])::forEach((chain) => {
		DIR_CHAIN[chain] = DIR_CHAIN[chain] || {};
		DIR_CHAIN[chain][name] = true;
	});

	(Array.isArray(params.end) ? params.end : [params.end])::forEach((end) => {
