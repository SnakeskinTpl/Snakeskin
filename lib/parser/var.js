/*!
 * Snakeskin
 * https://github.com/SnakeskinTpl/Snakeskin
 *
 * Released under the MIT license
 * https://github.com/SnakeskinTpl/Snakeskin/blob/master/LICENSE
 */

import Parser from './constructor.js';

/**
 * Declares a variable and returns a string of declaration
 *
 * @param {string} varName - the variable name
 * @param {boolean=} [opt_callParams=false] - if is true, then the variable
 *   will be declared as a function parameter
 *
 * @return {string}
 */
Parser.prototype.declVar = function (varName, opt_callParams) {
	opt_callParams = opt_callParams || false;

	const
		tplName = this.tplName,
		id = this.module.id;

	let
		struct = this.structure;

	if (!opt_callParams && tplName && constCache[tplName][varName]) {
		this.error(`the variable "${varName}" is already defined as constant`);
	}

	while (!struct.vars) {
		struct = struct.parent;
	}

	const val = struct.vars[varName];
	if (val && !val.inherited && struct.parent) {
		return val.value;
	}

	let
		realVar,
		global = false;

	if (struct.name === 'root' || this.getGroup('import')[struct.name]) {
		if (struct.name !== 'root') {
			struct = struct.parent;
		}

		realVar = `__LOCAL__.${varName}_${id}_${uid}`;
		varName += `_${id}`;
		global = true;

	} else {
		realVar = `__${varName}_${this.proto ? this.proto.name : ''}_${struct.name}_${this.i}`;
	}

	struct.vars[varName] = {
		value: realVar,
		id: id,
		global: global,
		scope: this.scope.length
	};

	if (tplName) {
		this.varCache[tplName][varName] = true;
	}

	return realVar;
};

/**
 * Parses a string of variables declaration, initializes it and returns
 * a string of declaration
 *
 * @param {string} str - the source string
 * @param {?boolean=} [opt_end=true] - if is true, then will be appended ; to the string
 * @param {?string=} [opt_init] - a value of a variable by default
 * @return {string}
 */
Parser.prototype.multiDeclVar = function (str, opt_end, opt_init) {
	opt_end = opt_end !== false;
	opt_init = opt_init == null ?
		'void 0' : opt_init;

	let
		isSys = 0,
		cache = '';

	let
		fin = 'var ',
		length = str.length,
		struct = this.structure;

	while (!struct.vars) {
		struct = struct.parent;
	}

	if (struct.name === 'root' || this.getGroup('import')[struct.name]) {
		fin = '';
	}

	for (let i = -1; ++i < length;) {
		const el = str.charAt(i);

		if (bMap[el]) {
			isSys++;

		} else if (closeBMap[el]) {
			isSys--;
		}

		if ((el === ',' || i === length - 1) && !isSys) {
			if (i === length - 1) {
				cache += el;
			}

			let parts = cache.split('='),
				realVar = this.declVar(parts[0].trim());

			parts[0] = realVar + (opt_init || parts[1] ? '=' : '');
			parts[1] = parts[1] || opt_init;

			let val = parts.slice(1).join('=');
			fin += parts[0] + (val ? this.prepareOutput(val, true) : '') + ',';

			cache = '';
			continue;
		}

		cache += el;
	}

	if (isSys) {
		this.error(`invalid "${this.name}" declaration`);
	}

	return fin.slice(0, -1) + (opt_end ? ';' : '');
};

/**
 * Declares an object of arguments
 * and returns a string of declaration
 * @return {string}
 */
Parser.prototype.declArguments = function () {
	return /* cbws */`
		var __ARGUMENTS__ = arguments;
		${this.multiDeclVar('arguments = __ARGUMENTS__')}
	`;
};
