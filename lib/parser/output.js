					...
					if (!pCount) {
						if (nres) {
							nres =
								...
								(pContent && this.out(pContent, true, null, null, true)) +
 ...
 * @param {?boolean=} [opt_iSys=false] - if is true, then call is considered as deep system
 * @param {?boolean=} [opt_breakFirst=false] - if is true, then the first word in the string will be skipped
 * @param {?boolean=} [opt_validate=true] - if is false, then the resulting string won't be validated
 * @return {string}
 */
Parser.prototype.out = function (command, opt_sys, opt_iSys, opt_breakFirst, opt_validate) {
	const
	...
	for (let i = 0; i < commandLength; i++) {
		...
		if (!breakNum) {
			...
			if (nword && !posNWord && nextCharRgxp.test(el)) {
				const
					nextStep = this.getWord(command, i),
					word = nextStep.word,
		...
		if (el === ')' && pCountFilter && !breakNum) {
			...
			if (!pCountFilter) {
				...
				filter[last] = this.out(cache, true, null, true, false);
				const
