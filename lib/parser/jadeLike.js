...
/**
 * Returns a template description object
...
function getLineDesc(str, i, dir, comment) {
	...
	for (let j = i - 1; ++j < str.length;) {
		...
		if (rgxp.eol.test(el)) {
			...
			if (comment || (sComment && concatLine)) {
				...
				const dirStart = rgxp.whitespace.test(
					str.charAt(j - 2)
				);
				...
				if (dirStart && (prevEl === CONCAT && res !== CONCAT || brk)) {
					literal = prevEl;
	...
	if (dir && lastEl === CONCAT_END && rgxp.whitespace.test(res.charAt(lastElI - 1))) {
