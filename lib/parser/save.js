 ...
 * @return {string}
 */
Parser.prototype.returnPushNodeDecl = function (opt_inline) {
	return /* cbws */`
		${this.wrap('__NODE__')}
 ...
 * @return {string}
 */
Parser.prototype.declResult = function () {
	switch (this.renderMode) {
		case 'stringBuffer':
...
Parser.prototype.end = function (cacheKey, label) {
	...
	this.res = this.pasteDangerBlocks(this.res)
		.replace(
			/__CDATA__(\d+)_/g,
			(sstr, pos) => escapeEOLs(
					this.cDataContent[pos].replace(new RegExp(nextLineRgxp.source, 'g'), this.eol)
				).replace(/'/g, '&#39;')
		);
