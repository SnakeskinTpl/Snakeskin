			...
			if (splitDeclRgxp.test(command)) {
				...
				if (this.isReady()) {
					let decl = varDeclRgxp.test(parts[0]) ?
						this.declVars(parts[0].replace(varDeclRgxp, '')) : this.out(parts[0], true);

					parts[1] = parts[1] && `(${parts[1]})`;
					parts[2] = parts[2] && `(${parts[2]})`;

					this.append(`for (${decl + this.out(parts.slice(1).join(';'), true)}) {`);
				}
				...
				if (this.isReady()) {
					let decl = parts[1] ?
						this.declVars(parts[2], false, '') : this.out(parts[2], true);

					this.append(`for (${decl} ${parts[3]} ${this.out(parts[4], true)}) {`);
				}
			}
		},
	...
	Snakeskin.addDirective(
		...
		function (command) {
			if (this.structure.name == 'do') {
				...
				if (this.isReady()) {
					this.append(`} while (${this.out(command, true)});`);
				}
