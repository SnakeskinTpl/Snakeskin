				...
				if (tplName && type !== 'global') {
					...
					if (this.isReady()) {
						...
						if (output) {
							...
							this.append(`${prop} = ${this.out(parts.slice(1).join('='), true)};`);
						}
					}
					...
					if (output && tplName) {
						this.text = true;
						this.append(this.wrap(`${this.out(desc.key, true)} = ${this.out(desc.value)};`));

					} else {
						this.save(`${this.out(command, true)};`);
					}
				}
				...
				if (this.isReady()) {
					...
					if (desc) {
						if (output) {
							...
							this.text = false;
							this.append(`${this.out(command, true)};`);
						}
