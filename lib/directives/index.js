...
Snakeskin.addDirective = function (name, params, constr, opt_destr) {
	...
	if (p.placement && !new RegExp(`${r(alb)}?`).test(p.placement)) {
		DIR_PLACEMENT[name] = $C([].concat(p.placement)).reduce((map, el) =>
			(map[el] = [el], map), {});

		$C(DIR_PLACEMENT).forEach((map, key) => {
			...
			DIR_PLACEMENT_PLAIN[key] = {};
			$C(map).forEach((el) => {
				$C(el).forEach((el) => {
					DIR_PLACEMENT_PLAIN[key][el] = true;
				});
			});
		});
	}
	...
	Snakeskin.Directives[name] = function (command, commandLength, type, jsDoc) {
		...
		switch (p.placement) {
			...
			default:
				if (p.placement && !parser.hasParent(DIR_PLACEMENT_PLAIN[name])) {
					parser.error(
