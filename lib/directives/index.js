...
Snakeskin.addDirective = function (name, params, constr, opt_destr) {
	...
	const
		placementRgxp = new RegExp(`${r(alb)}?`);
	...
	Snakeskin.Directives[name] = function (command, commandLength, type, jsDoc) {
		...
		switch (p.placement) {
			...
			default:
				const
					isGroup = placementRgxp.test(p.placement);

				p.placement = isGroup ?
					p.placement.replace(placementRgxp, '') : 0;

				if (p.placement && parser.hasParent(p.placement)) {
					parser.error(
						`"${dirName}" can be used only within: ${isGroup ? q(parser.getGroupList(p.placement)) : p.placement}`
					);
				}
		}
		...
		if (p.chain && !$C(p.chain).some((el) => struct.name === el)) {
			const groups = $C(p.chain).reduce((arr, el) => arr.concat(DIR_GROUPS_LIST[el]), []);
			return parser.error(`"${dirName}" can be used only with a ${groups.join(', ')}`);
		}
