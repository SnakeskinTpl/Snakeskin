...
Snakeskin.addDirective = function (name, params, constr, opt_destr) {
	...
	let
		_ = ([cache, plainCache, val]) => ({cache, plainCache, val});
	...
	$C(p.replacers).forEach((el, key) => {
		...
		if (DIR_NAME_REPLACERS[key]) {
			throw new Error(`Replacer "${key}" already exists`);
		}
	...
	Snakeskin.Directives[name] = function (command, commandLength, type, jsDoc) {
		...
		switch (p.placement) {
			case `${alb}template`:
				if (!struct.parent) {
					return parser.error(
						`"${dirName}" can be used only within: ${q(parser.getGroupList('template'))}`
					);
				}
			...
			case `${alb}global`:
				if (struct.parent) {
					return parser.error(`"${dirName}" can be used only within the global space`);
				}
			...
			default:
				if (p.placement && !parser.has(DIR_PLACEMENT_PLAIN[name])) {
					return parser.error(
						`"${dirName}" can be used only within: ${q(Object.keys(DIR_PLACEMENT_PLAIN[name]))}`
					);
				}
		}

		if (p.notEmpty && !command) {
			return parser.error(`"${dirName}" must have a body`);
		}

		if (p.generator && !parser.parentTplName && !parser.generator && !parser.proto && !parser.outerLink) {
			return parser.error(`"${dirName}" can be used only with a generator`);
		}

		if (p.chain && !DIR_CHAIN[parentDirName][dirName]) {
			const groups = $C(p.chain).reduce((arr, el) =>
				arr.concat(el[0] === lb ? parser.getGroupList(el) : el), []);

			return parser.error(`"${dirName}" can be used only with a ${q(groups)}`);
		}

		if (p.blacklist && parser.has(DIR_BLACKLIST_PLAIN[name])) {
			return parser.error(`"${dirName}" can't be used with: ${q(Object.keys(DIR_BLACKLIST_PLAIN[name]))}`);
		}
		...
		if (struct.strong) {
			if (DIR_INSIDE[struct.name][dirName]) {
				...
				return parser.error(`"${dirName}" can't be used within the "${struct.name}"`);
			}
		}

		if (!p.selfInclude && parser.has(dirName)) {
			return parser.error(`"${dirName}" can't be used within the "${dirName}"`);
		}
		...
		if (dirName === sourceName) {
			if (struct === newStruct) {
				if (!ignore && DIR_AFTER[struct.name] && !DIR_AFTER[struct.name][dirName]) {
					return parser.error(`"${dirName}" can't be used after the "${struct.name}"`);
				}
				...
				if (siblings) {
					...
					if (!ignore && prev && DIR_AFTER[prev.name] && !DIR_AFTER[prev.name][dirName]) {
						return parser.error(`"${dirName}" can't be used after the "${prev.name}"`);
					}
				}
			}
		}
