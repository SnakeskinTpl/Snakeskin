	...
	function (command) {
		...
		if (this.isReady()) {
			if (command === 'proto') {
				...
				this.append(this.out('break __I_PROTO__;', true));
				return;
			}
			...
			if (cycles[inside]) {
				...
				let val = command ? this.out(command, true) : 'false';
	...
	function (command) {
		...
		if (this.isReady()) {
			if (command === 'proto') {
				...
				this.append(this.out('continue __I_PROTO__;', true));
				return;
			}
			...
			if (cycles[inside]) {
				...
				let val = command ? `null,${this.out(command, true)}` : '';
