...
DirObj.prototype.returnProtoArgs = function (protoArgs, args) {
	...
	for (let i = -1; ++i < length;) {
		let val = this.out(args[i] || 'void 0', true);

		let arg = protoArgs[i][0],
			def = protoArgs[i][1];

		if (def !== undefined) {
			def = this.out(def, true);
		}
		...
		if (protoArgs['__SNAKESKIN_TMP__needArgs'] && i === length - 1) {
			...
			str += ws`
				var ${arg} = ${def !== undefined ?
					val ? `${val} != null ? ${val} : ${this.out(def, true)}` : def : val || 'void 0'
				};
			`;
		}
	}
	...
	Snakeskin.addDirective(
		...
		function (command) {
			...
			if (this.isSimpleOutput()) {
				let i = this.out('__I_PROTO__', true);
				protoCache[this.tplName][this.proto.name].i = i;
				this.save(`${i}:while (${this.out(command, true)}) {`);
			}
		},
