...
$C(['template', 'interface', 'placeholder']).forEach((template) => {
	Snakeskin.addDirective(
...

		{
			deferInit: true,
			block: true,
			placement: Snakeskin.template('global'),
			notEmpty: true,
			group: [
				'template',
				'rootTemplate',
		...
		function (command, commandLength, type, jsDoc) {
			...
			if (!proto) {
				...
				this.save(
					(pos = `/* Snakeskin template: ${tplName}; ${fnArgsKey} */`),
					iface,
					jsDoc
				);
			...
			if ((!extMap[tplName] || parentTplName) && preDefs) {
				this.source = this.source.slice(0, this.i + 1) +
					preDefs.text +
		...
		function (command, commandLength) {
			...
			if (this.parentTplName) {
				...
				this.source = this.source.slice(0, this.startTemplateI) +
			...
			if (this.backTableI) {
				forIn(this.backTable, (arr, key) => {
					for (let i = -1; ++i < arr.length;) {
						...
						this.res = this.res.slice(0, el.pos) +
							this.res.slice(el.pos).replace(
								el.label,
