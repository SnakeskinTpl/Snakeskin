		...
		function (command) {
			...
			if (this.isReady()) {
				if (!this.inlineIterators) {
					if (parts.length === 3) {
						this.append(ws`
							${this.out(`$C(${parts[0]})`, true)}.forEach(function (${this.declCallbackArgs(parts)}) {
								${this.declArguments()}
						`);
					...
					objLength += ws`
						...
						if (!${keys}) {
							...
							for (${this.declVars('__KEY__', false)} in ${cacheObj}) {
								if (!${cacheObj}.hasOwnProperty(${this.out('__KEY__', true)})) {
									continue;
								}

								${this.out('__LENGTH__++;', true)}
							}
						}
					`;
				}
				...
				let resStr = ws`
					...
					if (${cacheObj}) {
						if (Array.isArray(${cacheObj})) {
							...
							for (${this.declVars('__I__ = -1') + this.out('++__I__ < __LENGTH__;', true)}) {
				`;
		...
		function () {
			if (this.isReady()) {
				...
				if (this.inlineIterators) {
					...
					if (params.params) {
						this.append(`}, ${this.out(params.params, true)});`);
