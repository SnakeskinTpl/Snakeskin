...
for (var i = 0; i < template.length; i++) {
	Snakeskin.addDirective(
		...
		function (command, commandLength, type, jsDoc) {var this$0 = this;
			...
			var replaceFileName = function(tplName)  {
				if (IS_NODE && file) {
					var path = require('path');
					tplName = this$0.replaceDangerBlocks(tplName.replace(/(.?)%fileName%/g, function(sstr, $1)  {
...
\n							")),
			...
			if (/\bextends\b/m.test(command)) {
				...
				if (nmRgxp.test(parentTplName)) {
					var tmpArr$0 = parentTplName
						.replace(nmsRgxp, '.%')
						.replace(nmeRgxp, '')
						.split('.');

					var str$0 = tmpArr$0[0],
						length$0 = tmpArr$0.length;

					for (var i$0 = 1; i$0 < length$0; i$0++) {
						var el$0 = tmpArr$0[i$0],
							custom$0 = el$0.charAt(0) === '%';
						...
						if (custom$0) {
							str$0 += (("['" + (this.evalStr(("return " + (this.pasteDangerBlocks(el$0)))))) + "']");
							continue;
						}
				...
				for (var key in parentArgs) {
					...
					if (el$1.value !== void 0) {
						if (!argsTable[key]) {
							...
							argsTable[key].value = el$1.value;
						}
					}
				}
			}
			...
			for (var key$0 in argsTable) {
				...
				var el$2 = argsTable[key$0];

				if (el$2.local) {
					localVars[el$2.i] = el$2;

				} else {
					argsList[el$2.i] = el$2;
				}
			}
			...
			var defParams = '';
			for (var i$2 = 0; i$2 < argsList.length; i$2++) {
				var el$3 = argsList[i$2];
				el$3.key = el$3.key.replace(scopeModRgxp, '');

				this.save(el$3.key, iface);
				constICache[tplName][el$3.key] = el$3;

				if (el$3.value !== void 0) {
					defParams += (("" + (el$3.key)) + (" = " + (el$3.key)) + (" != null ? " + (el$3.key)) + (" : " + (this.prepareOutput(el$3.value, true))) + ";");
				}
			...
			var defs = '';
			for (var i$3 = 0; i$3 < localVars.length; i$3++) {
				var el$4 = localVars[i$3];

				if (!el$4) {
					continue;
				}

				defs += (("" + (this.needPrfx ? ALB : '')) + ("{__const__ " + (el$4.key.replace(scopeModRgxp, ''))) + (" = " + (el$4.value)) + "}");
			}
		...
		function (command, commandLength) {
			...
			if (this.backTableI) {
				...
				for (var key$1 in cache$1) {
					...
					for (var i$4 = 0; i$4 < cache$1[key$1].length; i$4++) {
						var el$5 = cache$1[key$1][i$4];

						if (!el$5.outer) {
							continue;
						}

						var tmp = protoCache[tplName][key$1];
						if (!tmp) {
							return this.error((("proto \"" + key$1) + "\" is not defined"));
						}

						this.res = this.res.substring(0, el$5.pos) +
							this.res.substring(el$5.pos).replace(
								el$5.label,
									(el$5.argsStr || '') + (el$5.recursive ? tmp.i + '++;' : tmp.body)
							);
					}
				}
