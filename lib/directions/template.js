		...
		function (command, commandLength, type, jsDoc) {
			...
			if (proto) {
				...
				if (IS_NODE && file) {
					...
					command = this.replaceDangerBlocks(command.replace(/(.?)%fileName%/g, function(sstr, $1)  {
...
\n							")),
			...
			for (var i$2 = 0; i$2 < localVars.length; i$2++) {
				...
				defs += (("" + (this.needPrfx ? ALB : '')) + ("{__const__ " + (el$3.key.replace(scopeModRgxp, ''))) + (" = " + (el$3.value)) + "}");
			}
					...
					for (var i$3 = 0; i$3 < cache$1[key$1].length; i$3++) {
						...
						this.res = this.res.substring(0, el$4.pos) +
							this.res.substring(el$4.pos).replace(
								el$4.label,
									(el$4.argsStr || '') + (el$4.recursive ? tmp.i + '++;' : tmp.body)
							);
					}
				}
