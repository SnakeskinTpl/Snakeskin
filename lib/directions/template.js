...
Snakeskin.addDirective(
	...
	(templateStart = function (command, commandLength, type, jsDoc) {
...
\n					")),
		...
		if (parentArgs) {
			for (var key in parentArgs) {
				...
				var cVal = current && current.value === void 0;
		...
		for (var i$1 = 0; i$1 < argsList.length; i$1++) {
			...
			el$2.key = el$2.key.charAt(0) === '@' ?
				el$2.key.substring(1) : el$2.key;
		...
		for (var i$2 = 0; i$2 < localVars.length; i$2++) {
			...
			defs += (("{__const__ " + (el$3.key.charAt(0) === '@' ? el$3.key.substring(1) : el$3.key)) + (" = " + (el$3.value)) + "}");
		}
...
\n		"));
	...
	(templateEnd = function (command, commandLength) {
