...
DirObj.prototype.replaceTplVars = function (str) {
	...
	var replacer = function(str)  {return str.replace(/\\/gm, '\\\\').replace(/('|")/gm, '\\$1')};
...
DirObj.prototype.prepareOutput = function (command, opt_sys, opt_isys, opt_breakFirst, opt_validate) {
	...
	var replacePropVal = function(sstr)  {return vars[sstr] ? vars[sstr].value : sstr};
			...
			if (nword && !posNWord && nextCharRgxp.test(el)) {
				...
				// Экспорт глобальный и супер глобальных переменных
					...
					// Супер глобальная переменная вне with
						...
						vres = varsNm + setMod(finalWord.substring(next === '@' ? 2 : 1));
					}
	...
	if (opt_validate !== false) {
		try {
			...
			return this.error(err.message.replace(/.*?: (\w)/, function(sstr, $1)  {return $1.toLowerCase()}));
		}
	}

	return (!unEscape && !opt_sys ? (("" + filtersNm) + ".html(") : '') + res + (!unEscape && !opt_sys ? ')' : '');
};