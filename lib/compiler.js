...
Snakeskin.compile = function (src, opt_params, opt_info, opt_sysParams) {
	...
	while (++dir.i < dir.source.length) {
		...
		if (i18Block ? begin : !begin) {
			if ((i18Back || i18nStart) && !escape && el === '"') {
				el = '\\"';
			}
			...
			if (i18Back) {
				if (dir.isSimpleOutput()) {
					...
					dir.save(dir.applyDefEscape(("i18n(\"" + el)));
					i18nStr = el;
				}
		...
		if (!bOpen) {
			...
			if (!jsDoc) {
				var isPrefStart = !currentEscape && el === pref && next === '{';
	...
	dir.res = dir.pasteDangerBlocks(dir.res)
		...
		.replace(
			/__CDATA__(\d+)_/g,
			function(sstr, pos)  {return dir.escapeNextLine(dir.cDataContent[pos]).replace(/'/gm, '&#39;')}
		);
	...
	for (var key$0 in dir.preProtos) {
		...
		dir.error((("template \"" + key$0) + "\" is not defined"));
		return false;
	}
