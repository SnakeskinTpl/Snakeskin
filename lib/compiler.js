...
Snakeskin.compile = function (src, opt_params, opt_info, opt_sysParams) {
	...
	// If is true, then a directive declaration has been started,
	// ie { ... }
	var begin = false,
		pseudoI = false;
	...
	// A count of open { symbols inside a directive
	var fakeBegin = 0;

	// If is true, then a string declaration has been started
	var beginStr = false;

	// If is true, then a previous symbol hasn't been escaped
	var escape = false;

	// If isn't false, then a comment block has been started (///, /*, /**)
	var comment = false,
		commentStart = 0;

	// If is true, then JSDoc has been started
	var jsDoc = false,
		jsDocStart = false;
			...
			if (jsDoc) {
				...
				if (dir.isSimpleOutput()) {
					...
					if (dir.doctype) {
						...
						// XML attribute has been started
						} else if (tOpen && el === '=') {
							tAttr = true;
						}
	...
	// then templates will be attached as a external script
	if (!IS_NODE) {
		setTimeout(() => {
			try {
