...
Snakeskin.compile = function (src, opt_params, opt_info, opt_sysParams) {
		...
		if (!bOpen) {
			...
			if (!jsDoc) {
				...
				// A directive is ended
					...
					if (!parser.proto && commandType.charAt(0) === '_') {
						const
							source = `${alb}?${lb}__.*?__.*?${rb}`,
					...
					command = parser.replaceDangerBlocks((isConst || commandType !== 'const' ?
						command.replace(commandRgxp, '') : command));

					parser.space = parser.prevSpace;
					const fnRes = Snakeskin.Directions[commandType].call(
						parser,
						command,
						commandLength,
						commandType,
					...
					if (parser.language) {
						...
						if (i18nStart) {
							...
							if (begin) {
								...
								parser.source = str.substring(0, parser.i + 1) +
									advStr +
							...
							if (begin) {
								...
								const diff = Number(parser.needPrfx) + 1;

								parser.source = str.substring(0, parser.i) +
									(parser.needPrfx ? alb : '') +
									lb +
		...
		if (begin) {
			...
			if (jsDoc) {
				...
				if (currentClrL && !parser.tplName && (SHORTS[el] || SHORTS[next2str])) {
					let adv = parser.lines[lastLine].length - 1,
					...
					parser.source = parser.source.substring(0, parser.i - adv) +
						source.str +
				...
				parser.error('the text can\'t be used in the global space');
				return false;
				...
				if (parser.isSimpleOutput()) {
					...
					if (parser.autoReplace) {
						...
						if (parser.macros[expr]) {
							let modStr = parser.res.substring(0, exprPos) +
								parser.res.substring(exprPos + expr.length + advExprPos);

							let val = parser.macros[expr].call ?
	...
	saveCache(cacheKey, text, p, parser);
