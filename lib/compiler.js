...
import * as rgxp from './consts/regs.js';
...
Snakeskin.compile = function (src, opt_params, opt_info, opt_sysParams) {
	...
	const
		info = _.any($C.extend(false, {line: 1}, opt_info || {}));
	...
	if (p.cache) {
		const
			tmp = returnCache(cacheKey, text, p, ctx);
	...
	if (p.prettyPrint) {
		...
		dir.res = dir.res.replace(new RegExp(nextLineRgxp.source, 'g'), eol);
	}
