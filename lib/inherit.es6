...
DirObj.prototype.getExtStr = function (tplName) {
	...
	for (let i = 0; i < 6; i++) {
		...
		for (let key in el) {
			...
			let block = cache[tplName].substring(current.from, current.to);
			...
			// Добавление
				...
				let _ = current.needPrfx ?
					PRFX : '';

				if (i === 1) {
					res += `${_}{block ${key}}${block}${_}{end}`;
				...
				// Переменные и прототипы
					...
					block = i === 3 ?
						`${_}{${block}}` : `${_}{proto ${key + current.argsDecl}}${block}${_}{end}`;
