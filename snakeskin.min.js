...
"(").replace(/__SNAKESKIN_ESCAPE__BCLOSE/g,")")};Snakeskin.compile=function(b,h,c){b=String(b.innerHTML||b).replace(/{cdata}([\s\S]*?){end\s+cdata}/gm,function(a,b){N.push(b);return"__SNAKESKIN_CDATA__"+(N.length-1)}).replace(/\/\/.*/gm,"").replace(/[\t\v\n\r]*/gm,"").replace(/\/\*[\s\S]*?\*\//g,"");for(var a=""+(!c?"/* This code is generated automatically, don't alter it. */":"")+(h?"var Snakeskin = global.Snakeskin;exports.liveInit = function (path) { Snakeskin = require(path);exec();return this;};function exec() {":
""),e,d=null,f,k,p,m,x=0,l=0,y,q=[],w=[],z,s,A=[],H,I=[],P,Q={},g="",M,r,R,n=-1,t,D;++n<b.length;){e=b.charAt(n);if(!D)if("{"===e)if(m)x++;else{m=!0;continue}else if("}"===e&&(!x||!x--)){m=!1;g=this._escape(g);switch(g.split(" ")[0]){case "template":if(0!==l)throw j=Error('Missing closing or opening tag in the template (template: "'+d+'")!'),j.name="Snakeskin Error",j;l++;t=n+1;d=g.replace(/^template\s+/,"").replace(/\(.*/,"");if(c)break;/\s+extends\s+/.test(g)&&(f=g.replace(/.*?\s+extends\s+(.*)/,
"$1"));k=g.replace(/.*?\((.*?)\).*/,"$1");B[d]={};v[d]={};J[d]=0;C[d]={};K[d]=0;G[d]={};L[d]=f;a+="/* Snakeskin template: "+d+"; "+k.replace(/=(.*?)(?:,|$)/g,"")+" */";/\./.test(d)||h?(d.split(".").reduce(function(b,c,d){Q[b]||(a+="if (typeof "+(h?"exports.":"")+b+" === 'undefined') { "+(h?"exports.":1===d?E?"var ":"window.":"")+b+" = {}; }",Q[b]=!0);return b+"."+c}),a+=(h?"exports.":"")+d+"= function ("):a+=(!E?"window."+d+" = ":"")+"function "+(E?d:"")+"(";k=k.split(",");u[d]=u[f]?u[f].concat(k):
k;p="";u[f]&&u[f].forEach(function(a){var b=a.split("=");b[0]=b[0].trim();b[1]=b[1]&&b[1].trim();k.forEach(function(c,d){var f=c.split("=");f[0]=f[0].trim();f[1]=f[1]&&f[1].trim();b[0]===f[0]&&"undefined"===typeof f[1]&&(k[d]=a)})});k.forEach(function(b,c){var e=b.split("=");e[0]=e[0].trim();a+=e[0];1<e.length&&(u[f]&&!p&&u[f].forEach(function(a){var b=a.split("=");b[0]=b[0].trim();b[1]=b[1]&&b[1].trim();!k.some(function(a){a=a.split("=");a[0]=a[0].trim();a[1]=a[1]&&a[1].trim();return a[0]===b[0]})&&
"undefined"!==typeof b[1]&&(p+="var "+b[0]+" = typeof "+b[0]+" !== 'undefined' && "+b[0]+" !== null ? "+b[0]+" : "+b[1]+";",G[d][b[0]]=a)}),e[1]=e[1].trim(),p+=e[0]+" = typeof "+e[0]+" !== 'undefined' && "+e[0]+" !== null ? "+e[0]+" : "+e[1]+";");G[d][e[0]]=b;c!==k.length-1&&(a+=",")});a+=") { "+p+"var __SNAKESKIN_RESULT__ = '', __SNAKESKIN_TMP_RESULT__;";break;case "if":l++;!f&&!s&&(a+="if ("+g.replace(/^\/?if\s+/,"")+") {");break;case "elseIf":!f&&!s&&(a+="} else if ("+g.replace(/^elseIf\s+/,"")+
") {");break;case "else":!f&&!s&&(a+="} else {");break;case "with":A.push({scope:g.replace(/^with\s+/,""),i:++l});break;case "proto":if(!c&&(f&&!w.length||!f)){if(v[d][g])throw j=Error('Proto "'+g.replace(/^proto\s+/,"")+'" is already defined (template: "'+d+'")!'),j.name="Snakeskin Error",j;v[d][g]={from:n-t+1}}w.push({name:g,i:++l,startI:n+1});f||(s=!0);break;case "apply":if(!f&&!w.length){g=g.replace(/^apply/,"proto");if(!v[d][g])throw j=Error('Proto "'+g.replace(/^proto\s+/,"")+'" is not defined (template: "'+
