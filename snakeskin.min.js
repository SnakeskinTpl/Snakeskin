var Snakeskin={VERSION:"1.3.3",Filters:{},Vars:{BEMtag:{}},cache:{}},SS=Snakeskin;
...
Error('Variable "'+p+"\" can't be defined in a loop (command: {"+d+'}, template: "'+e+", "+this._genError(h)+'")!'),k.name="Snakeskin Error",k;F[e][p]={from:q-v-d.length,to:q-v};P[e]=q-v+1;!g&&!t&&(b+="var "+d+";")}else U[p]=!0,b+="if (typeof Snakeskin !== 'undefined') { Snakeskin.Vars."+d+"; }";else!g&&!t&&(p="",Y=1,R=!1,d.replace(/\|\|/g,"__SNAKESKIN_ESCAPE__OR").split("|").forEach(function(a,b){var c,d;0===b?(C.length?(C.push({scope:a}),p=C.reduce(function(a,b){return("undefined"===typeof a.scope?
a:a.scope)+"."+b.scope}),C.pop()):p=a,p="Snakeskin.Filters.undef("+(!F[e][p]&&U[p]?"Snakeskin.Vars.":"")+p+")"):(c=a.split(" "),d=c.slice(1),"!html"!==c[0]?(Y++,p="Snakeskin.Filters['"+c[0]+"']("+p+(d.length?", "+d.join(""):"")+")"):R=!0)}),b+="__SNAKESKIN_RESULT__ += "+(!R?"Snakeskin.Filters.html(":"")+p+(!R?")":"")+";")}d="";continue}if(n){!t&&z&&(b+="';",z=!1);if((ia[j]||"/"===j)&&(!a[q-1]||"\\"!==a[q-1]))I&&I===j?I=!1:I||(I=j);d+=j}else t||(z||(b+="__SNAKESKIN_RESULT__ += '",z=!0),g||(b+=j))}b=
this._uescape(b,H).replace(/__SNAKESKIN_ESCAPE__OR/g,"||").replace(/__SNAKESKIN_CDATA__(\d+)/g,function(a,b){return S[b].replace(/\n/gm,"\\n").replace(/\r/gm,"\\r").replace(/\v/gm,"\\v").replace(/\'/gm,"&#39;")}).replace(/__SNAKESKIN_RESULT__ \+= '';/g,"");b+=!c?"/* Snakeskin templating system. Generated at: "+(new Date).toString()+". */":"";b+=f?"}":"";if(0!==l)throw k=Error("Missing closing or opening tag in the template (template: "+e+", "+this._genError(h)+")!"),k.name="Snakeskin Error",k;if(c)return b;
J?f?eval(b):global.eval(b):window.eval(b);return b};var D;if(J)for(D in Snakeskin)Snakeskin.hasOwnProperty(D)&&(exports[D]=Snakeskin[D])})("undefined"===typeof window);
