var Snakeskin={VERSION:"2.3",Directions:{},Filters:{},BEM:{},Vars:{},write:{},cache:{}};
...
i:++a.openBlockI},!0)};Snakeskin.Directions.withEnd=function(b,c,a){a.popPos("with")};Snakeskin.Directions["const"]=function(b,c,a,d){var e=a.tplName,f=a.parentTplName,g=a.protoStart,h=a.i,k=a.startI;if(f||g||!/^console\./.test(b))if(/^[a-z_][\w\[\].'"\s]*[^=]=[^=]/i.test(b)){var m=b.split("=")[0].trim();if(e){if(!(d.dryRun||a.varCache[m]||(!f||a.hasPos("block")||a.hasPos("proto"))&&f)){if(u[e][m]||B[e][m])throw a.error('Constant "'+m+'" is already defined (command: {'+b+'}, template: "'+e+", "+a.genErrorAdvInfo(d.info)+
'")!');if(W[m])throw a.error("Can't declare constant \""+m+'", try another name (command: {'+b+'}, template: "'+e+", "+a.genErrorAdvInfo(d.info)+'")!');if(a.hasPos("forEach"))throw a.error('Constant "'+m+"\" can't be defined in a loop (command: {"+b+'}, template: "'+e+", "+a.genErrorAdvInfo(d.info)+'")!');if(a.hasPos("with"))throw a.error('Constant "'+m+'" can\'t be defined inside a "with" block (command: {'+b+'}, template: "'+e+", "+a.genErrorAdvInfo(d.info)+'")!');u[e][m]={from:h-k-c,to:h-k};I[e]=
h-k+1}f||g||(a.varCache[m]?a.save(b+";",!0,!0):a.save(a.prepareOutput((/[.\[]/.test(m)?"":"var ")+b+";",!0)))}else a.save("if (typeof Snakeskin !== 'undefined') { Snakeskin.Vars."+a.prepareOutput(b,!0,!0)+"; }")}else f||g||a.save("__SNAKESKIN_RESULT__ += "+a.prepareOutput(b)+";");else a.save(b+";")};Snakeskin.Directions.cut=function(b,c,a){b=a.pasteDangerBlocks(b,a.quotContent);Snakeskin.write[b]||(Snakeskin.write[b]=!1)};Snakeskin.Directions.save=function(b,c,a){Snakeskin.write[a.pasteDangerBlocks(b,
a.quotContent)]=!0};Snakeskin.Directions.setBEM=function(b,c,a){b=b.match(/(.*?),\s+(.*)/);Snakeskin.BEM[b[1]]=(new Function("return {"+a.pasteDangerBlocks(b[2],a.quotContent)+"}"))()};Snakeskin.Directions.bem=function(b,c,a){a.pushPos("bem",{i:++a.openBlockI,tag:/^\(/g.test(b)?/\((.*?)\)/.exec(b)[1]:null});c=a.getLastPos("bem");b=c.tag?b.replace(/^.*?\)([\s\S]*)/,"$1"):b;b=b.trim().split(",");var d=b[0];c.original=Snakeskin.BEM[d]&&Snakeskin.BEM[d].tag;a.parentTplName||a.protoStart||(b[0]+="'",b=
b.join(","),a.save("__SNAKESKIN_RESULT__ += '<"+(c.tag||c.original||"div")+' class="i-block" data-params="{name: \\\''+a.replaceTplVars(b)+"}\">';"))};Snakeskin.Directions.bemEnd=function(b,c,a){b=a.popPos("bem");a.parentTplName||a.protoStart||a.save("__SNAKESKIN_RESULT__ += '</"+(b.tag||b.original||"div")+">';")};Snakeskin.Directions.data=function(b,c,a){a.parentTplName||a.protoStart||a.save("__SNAKESKIN_RESULT__ += '"+a.replaceTplVars(b)+"';")};G&&(module.exports=Snakeskin)})("undefined"===typeof window);
