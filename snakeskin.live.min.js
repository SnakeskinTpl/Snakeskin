...
(Array.prototype.reduce=function(a,f){var c=0,h=this.length,b;if(1===h)return this[0];for(b="undefined"!==typeof f?f:this[0];++c<h;)b=a(b,this[c],c,this);return b});var J={},D={},w={},N={},U={},E={},O={},K={},x={},P={},R=[],ia={'"':!0,"'":!0},ja={__SNAKESKIN_RESULT__:!0,__SNAKESKIN_CDATA__:!0},k;Snakeskin._escape=function(a,f){return a.replace(/(["']).*?[^\\]\1/g,function(a){f&&f.push(a);return"__SNAKESKIN_QUOT__"+(f?f.length-1:"_")})};Snakeskin._uescape=function(a,f){return a.replace(/__SNAKESKIN_QUOT__(\d+)/g,
function(a,h){return f[h]})};Snakeskin._getExtStr=function(a,f){if("undefined"===typeof J[P[a]])throw k=Error('The specified pattern ("'+P[a]+'" for "'+a+'") for inheritance is not defined ('+this._genError(f)+")!"),k.name="Snakeskin Error",k;for(var c=P[a],h=J[c],b,j,e,g,m=-1,r,n=0,x=[],l,z,s;6>++m;)for(g in 0===m?(b=D[a],j=D[c]):2===m?(b=E[a],j=E[c],n=O[c],r=null):4===m&&(b=w[a],j=w[c],n=N[c],r=null),b)if(b.hasOwnProperty(g))if(s=0,e=J[a].substring(b[g].from,b[g].to),j[g]&&(l=e.length-J[c].substring(j[g].from,
...
x.push({val:r,adv:e.length}),n+=e.length;return h};Snakeskin._genError=function(a){var f,c="";for(f in a)a.hasOwnProperty(f)&&(c=a[f].innerHTML?c+(f+": (class: "+(a[f].className||"undefined")+", id: "+(a[f].id||"undefined")+"), "):c+(f+": "+a[f]+", "));return c.replace(/, $/,"")};Snakeskin.compile=function(a,f,c,h){h=h||{};a.innerHTML&&(h.node=a);a=String(a.innerHTML||a).replace(/{cdata}([\s\S]*?){end\s+cdata}/gm,function(a,b){R.push(b);return"__SNAKESKIN_CDATA__"+(R.length-1)}).replace(/\/\/.*/gm,
"").replace(/[\t\v\r\n]/gm,"").replace(/\/\*[\s\S]*?\*\//g,"").trim();for(var b=""+(!c?"/* This code is generated automatically, don't alter it. */":"")+(f?"var Snakeskin = global.Snakeskin;exports.liveInit = function (path) { Snakeskin = require(path);exec();return this;};function exec() {":""),j,e=null,g,m,r,n,S=0,l=0,z,s=[],A,y=[],u,t,T=0,V,B={},C=[],L,M=[],W,F=[],X={},d="",Q,p,Y,G=[],q=-1,v,H;++q<a.length;){j=a.charAt(q);if(!H)if("{"===j)if(n)S++;else{n=!0;continue}else if("}"===j&&(!S||!S--)){n=
!1;d=this._escape(d,G).trim();switch(d.split(" ")[0]){case "template":if(0!==l)throw k=Error("Missing closing or opening tag in the template (command: {"+d+'}, template: "'+e+", "+this._genError(h)+'")!'),k.name="Snakeskin Error",k;l++;v=q+1;e=j=/^template\s+(.*?)\(/.exec(d)[1];e=this._uescape(e,G);if(c)break;/\s+extends\s+/.test(d)&&(g=this._uescape(/\s+extends\s+(.*)/.exec(d)[1],G));m=d.replace(/.*?\((.*?)\).*/,"$1");D[e]={};w[e]={};N[e]=0;E[e]={};O[e]=0;K[e]={};P[e]=g;b+="/* Snakeskin template: "+
e+"; "+m.replace(/=(.*?)(?:,|$)/g,"")+" */";/\.|\[/.test(j)||f?(j.replace(/\[/g,".").replace(/]/g,"").split(".").reduce(function(a,d,e){X[a]||(b+="if (typeof "+(f?"exports.":"")+a+" === 'undefined') { "+(f?"exports.":1===e?I?"var ":"window.":"")+a+" = {}; }",X[a]=!0);return"__SNAKESKIN_QUOT__"===d.substring(0,18)?a+"["+d+"]":a+"."+d}),b+=(f?"exports.":"")+j+"= function ("):b+=(!I?"window."+j+" = ":"")+"function "+(I?j:"")+"(";m=m.split(",");x[e]=x[g]?x[g].concat(m):m;r="";x[g]&&x[g].forEach(function(a){var b=
...
" = "+b[1]+";",K[e][b[0]]=a)}),c[1]=c[1].trim(),r+=c[0]+" = typeof "+c[0]+" !== 'undefined' && "+c[0]+" !== null ? "+c[0]+" : "+c[1]+";");K[e][c[0]]=a;d!==m.length-1&&(b+=",")});b+=") { "+r+"var __SNAKESKIN_RESULT__ = '';";break;case "if":l++;!g&&!t&&(b+="if ("+d.replace(/^if\s+/,"")+") {");break;case "elseIf":!g&&!t&&(b+="} else if ("+d.replace(/^elseIf\s+/,"")+") {");break;case "else":!g&&!t&&(b+="} else {");break;case "with":C.push({scope:d.replace(/^with\s+/,""),i:++l});break;case "proto":if(!c&&
(g&&!s.length&&!y.length||!g)){if(w[e][d])throw k=Error('Proto "'+d.replace(/^proto\s+/,"")+'" is already defined (command: {'+d+'}, template: "'+e+", "+this._genError(h)+'")!'),k.name="Snakeskin Error",k;w[e][d]={from:q-v+1}}y.push({name:d,i:++l,startI:q+1});g||(t=!0);break;case "apply":!g&&!y.length&&(d=d.replace(/^apply/,"proto"),w[e][d]?b+=w[e][d].body:(B[d]||(B[d]=[],B[d].protoStart=t,V=d.replace(/^proto\s+/,""),T++),B[d].push(b.length)));break;case "block":if(!c&&(g&&!s.length&&!y.length||!g)){if(D[e][d])throw k=
Error('Block "'+d.replace(/^block\s+/,"")+'" is already defined (command: {'+d+'}, template: "'+e+", "+this._genError(h)+'")!'),k.name="Snakeskin Error",k;D[e][d]={from:q-v+1}}s.push({name:d,i:++l});break;case "forEach":M.push(++l);!g&&!t&&(d=d.replace(/^forEach\s+/,"").split("=>"),b+=d[0]+" && Snakeskin.forEach("+d[0]+", function ("+(d[1]||"")+") {");break;case "bem":F.push({i:++l,tag:/\(/g.test(d)?/\(.*?\)/.exec(d)[1]:null});j=F[F.length-1];d=j.tag?d.replace(/^.*?\)([\s\S]*)/,"$1"):d.substring(3);
d=d.trim().split(",");A=d[0];j.original=this.Vars.BEMtag[A];!g&&!t&&(d[0]+="'",d=d.join(","),b+="__SNAKESKIN_RESULT__ += '<"+(j.tag||j.original||"div")+' class="i-bem" data-params="{name: \\\''+this._uescape(d,G).replace(/\\/g,"\\\\").replace(/('|")/g,"\\$1")+"}\">';");break;case "end":l--;A=s[s.length-1];u=y[y.length-1];L=C[C.length-1];W=M[M.length-1];j=F[F.length-1];if(0===l){if(T)throw k=Error('Proto "'+V+'" is not defined (command: {'+d+'}, template: "'+e+", "+this._genError(h)+'")!'),k.name=
"Snakeskin Error",k;if(c)break;J[e]=a.substring(v,q-d.length-1);if(g){a=a.substring(0,v)+this._getExtStr(e,h)+a.substring(q-d.length-1);D[e]={};w[e]={};N[e]=0;E[e]={};O[e]=0;K[e]={};q=v-1;l++;z=n=g=!1;d="";continue}b+="return __SNAKESKIN_RESULT__; }; Snakeskin.cache['"+this._uescape(e,G).replace(/'/g,"\\'")+"'] = "+(f?"exports.":"")+e+";";e=null}else if(!g&&(!u||u.i!==l+1)&&(!A||A.i!==l+1)&&(!L||L.i!==l+1))W===l+1?(M.pop(),t||(b+="});")):j&&j.i===l+1?(F.pop(),t||(b+="__SNAKESKIN_RESULT__ += '</"+
(j.tag||j.original||"div")+">';")):t||(b+="};");if(A&&A.i===l+1&&(s.pop(),!c&&(g&&!s.length&&!y.length||!g)))D[e][A.name].to=q-v-d.length-1;if(u&&u.i===l+1){y.pop();if(!c&&(g&&!s.length&&!y.length||!g))w[e][u.name].to=q-v-d.length-1,N[e]=q-v+1;g||(w[e][u.name].body=this.compile("{template "+e+"()}"+a.substring(u.startI,q-d.length-1)+"{end}",null,!0));B[u.name]&&!B[u.name].protoStart&&(B[u.name].forEach(function(a){b=b.substring(0,a)+w[e][u.name].body+b.substring(a)}),delete B[u.name],T--);y.length||
(t=!1)}L&&L.i===l+1&&C.pop();break;default:if(!g&&!t&&/console\./.test(d)){b+=d+";";break}if(/=(?!=)/.test(d))if(p=d.split("=")[0].trim(),e){if(E[e][p]||K[e][p])throw k=Error('Variable "'+p+'" is already defined (command: {'+d+'}, template: "'+e+", "+this._genError(h)+'")!'),k.name="Snakeskin Error",k;if(ja[p])throw k=Error("Can't declare variable \""+p+'", try another name (command: {'+d+'}, template: "'+e+", "+this._genError(h)+'")!'),k.name="Snakeskin Error",k;if(M.length)throw k=Error('Variable "'+
p+"\" can't be defined in a loop (command: {"+d+'}, template: "'+e+", "+this._genError(h)+'")!'),k.name="Snakeskin Error",k;E[e][p]={from:q-v-d.length,to:q-v};O[e]=q-v+1;!g&&!t&&(b+="var "+d+";")}else U[p]=!0,b+="Snakeskin.Vars."+d+";";else!g&&!t&&(p="",Y=1,Q=!1,d.replace(/\|\|/g,"__SNAKESKIN_ESCAPE__OR").split("|").forEach(function(a,b){var c,d;0===b?(C.length?(C.push({scope:a}),p=C.reduce(function(a,b){return("undefined"===typeof a.scope?a:a.scope)+"."+b.scope}),C.pop()):p=a,p="Snakeskin.Filters.undef("+
(!E[e][p]&&U[p]?"Snakeskin.Vars.":"")+p+")"):(c=a.split(" "),d=c.slice(1),"!html"!==c[0]?(Y++,p="Snakeskin.Filters['"+c[0]+"']("+p+(d.length?", "+d.join(""):"")+")"):Q=!0)}),b+="__SNAKESKIN_RESULT__ += "+(!Q?"Snakeskin.Filters.html(":"")+p+(!Q?")":"")+";")}d="";continue}if(n){!t&&z&&(b+="';",z=!1);if((ia[j]||"/"===j)&&(!a[q-1]||"\\"!==a[q-1]))H&&H===j?H=!1:H||(H=j);d+=j}else t||(z||(b+="__SNAKESKIN_RESULT__ += '",z=!0),g||(b+=j))}b=this._uescape(b,G).replace(/__SNAKESKIN_ESCAPE__OR/g,"||").replace(/__SNAKESKIN_CDATA__(\d+)/g,
function(a,b){return R[b].replace(/\n/gm,"\\n").replace(/\r/gm,"\\r").replace(/\v/gm,"\\v").replace(/\'/gm,"&#39;")}).replace(/__SNAKESKIN_RESULT__ \+= '';/g,"");b+=!c?"/* Snakeskin templating system. Generated at: "+(new Date).toString()+". */":"";b+=f?"}":"";if(0!==l)throw k=Error("Missing closing or opening tag in the template (template: "+e+", "+this._genError(h)+")!"),k.name="Snakeskin Error",k;if(c)return b;I?f?eval(b):global.eval(b):window.eval(b);return b};if(I)for(key in Snakeskin)Snakeskin.hasOwnProperty(key)&&
(exports[key]=Snakeskin[key])})("undefined"===typeof window);
