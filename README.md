...
*Размещение*: только глобальная область

*Закрывающий end*: необходим

*Формула*: `{template [пространствоИмён]названиеШаблона([параметры через запятую]) [extends родительскийШаблон]}`
