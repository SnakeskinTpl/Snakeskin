...
*Директива*: `template`.
*Применение*: только в глобальной области.
...
то оно может содержать любые валидные JS символы. Также есть ещё одно небольшое отличие в
...
то этот параметр становится локальной переменной внутри шаблона.

	{template base(a = 1)}
	{end}

	/// Параметр "a" стал локальной переменной со значением a = 1
...
Директива декларирует область шаблона или прототипа, которая может быть переопределена в дочернем шаблоне.

	{template base()}
		Привет {block foo}Мир{end}!
	{end}
...
Название блока может состоять из любые символов (в том числе содержать пробелы, однако крайние пробелы срезаются).
Названия блоков лежат в отдельном пространстве имён, поэтому могут совпадать с названием прототипов, констант и т.д..
Директива может содержать в себе другие директивы, включая декларацию констант или прототипов.

	{template base()}
		{block foo}
			Привет {block child}Мир{end}!
		{end}
	{end}
...
### Переопределение констант и прототипов

Подобно блокам или входным параметрам прототипы и константы могут также переопределяться в дочернем шаблоне.
